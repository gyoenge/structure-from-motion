...
# vis
from utils.vis import draw_matches, draw_keypoints, visualize_three_image_matches, visualize_three_image_matches_withkpnp
...
def args_parser():
    ...
    parser.add_argument('--matching_threshold_knn', type=float, default=0.60)
    ...
    parser.add_argument('--three_point_threshold', type=float, default=1e-6)
    parser.add_argument('--three_point_inlier_threshold', type=float, default=1e-4)
...
def main():
    ...
    if 2 in args.step and not os.path.isfile(E_result_path):
        ...
        ## inlier visulization #####################################################
        img1 = cv2.imread(os.path.join(args.dataset_path, args.object, image_names[0]), cv2.IMREAD_COLOR)
        img2 = cv2.imread(os.path.join(args.dataset_path, args.object, image_names[1]), cv2.IMREAD_COLOR)
        img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
        img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
        kp1 = image_features[0]
        kp2 = image_features[1]
        # matches
        matching_result_img = draw_matches(img1, img2, kp1, kp2, image_matches[0], inlier_matches_idx)
        matching_result_img_path = os.path.join(output_path, 'initial_results', 'inlier_matching_results_init_images.jpg')
        cv2.imwrite(matching_result_img_path, matching_result_img)
        # each keypoints 
        random_colors = np.random.randint(0, 256, size=(len(inlier_p1), 3)).tolist()
        vis_img1 = draw_keypoints(img1, inlier_p1, random_colors)
        vis_img2 = draw_keypoints(img2, inlier_p2, random_colors)
        vis_out1 = os.path.join(output_path, 'initial_results', 'inliers_img1.jpg')
        vis_out2 = os.path.join(output_path, 'initial_results', 'inliers_img2.jpg')
        cv2.imwrite(vis_out1, vis_img1)
        cv2.imwrite(vis_out2, vis_img2)
        #############################################################################
    else:
    ...
    for img_idx in range(len(image_names)-2):
        ...
        if 5 in args.step and not os.path.isfile(before_bundle_results_path):
            ...
            add_camera_matrix, matched_3D, matched_kp3 = three_point_algorithm(image_matches[img_idx], 
                                                      image_matches[img_idx+1], 
                                                      points2matches_idx_list[img_idx], 
                                                      points_list[img_idx], 
                                                      image_features[img_idx+1], ## added
                                                      image_features[img_idx+2], 
                                                      camera_intrinsic, 
                                                      eng, 
                                                      threepoint_threshold=args.three_point_threshold,
                                                      threepoint_max_iter=args.three_point_ransac_iter)
            # example 3-matched points 
            print("### example ---------------------------------------")
            kp3_ = image_features[img_idx+2][matched_kp3[0]].pt
            p3d_ = points_list[img_idx][matched_3D[0]]
            match_ = image_matches[img_idx][points2matches_idx_list[img_idx][matched_3D[0]]]
            kp1_ = image_features[img_idx][match_.queryIdx].pt
            kp2_ = image_features[img_idx+1][match_.trainIdx].pt
            print(f"kp1 : {kp1_} \nkp2 : {kp2_} \nkp3 : {kp3_} \n3d : {p3d_}")
            ## 
            output = visualize_three_image_matches_withkpnp(cv2.imread(os.path.join(args.dataset_path, args.object, img_name0+".jpg")),
                                                   cv2.imread(os.path.join(args.dataset_path, args.object,img_name1+".jpg")), 
                                                   cv2.imread(os.path.join(args.dataset_path, args.object,img_name2+".jpg")), 
                                                   kp1_, kp2_, kp3_
                                                   )
            output_file = os.path.join(step_result_folder, "three_view_example_match.jpg")
            cv2.imwrite(output_file, output)
            # triangulation test 
            points_3d, inlier_idx = triangulate_points(camera_matrix_list[img_idx], 
                                                       camera_matrix_list[img_idx+1], 
                                                       np.array([kp1_]), 
                                                       np.array([kp2_]), 
                                                       camera_intrinsic)
            print(f"cam1-cam2 triangulation : {points_3d}")
            points_3d, inlier_idx = triangulate_points(camera_matrix_list[img_idx+1], 
                                                       add_camera_matrix, 
                                                       np.array([kp2_]), 
                                                       np.array([kp3_]), 
                                                       camera_intrinsic)
            print(f"cam2-cam3 triangulation : {points_3d}")
            print("---------------------------------------------------")
            ## visualize three image matches #########################################
            output = visualize_three_image_matches(cv2.imread(os.path.join(args.dataset_path, args.object, img_name0+".jpg")),
                                                   cv2.imread(os.path.join(args.dataset_path, args.object,img_name1+".jpg")), 
                                                   cv2.imread(os.path.join(args.dataset_path, args.object,img_name2+".jpg")), 
                                                image_features[img_idx], image_features[img_idx+1], image_features[img_idx+2],
                                                image_matches[img_idx], image_matches[img_idx+1], 
                                                points2matches_idx_list[img_idx], matched_3D, matched_kp3)
            output_file = os.path.join(step_result_folder, "three_view_full_match.jpg")
            cv2.imwrite(output_file, output)
            ##########################################################################
            ...
            inlier_p1, inlier_p2, inlier_matches_idx = calculate_inlier_points(camera_matrix_list[img_idx+1],
                                                                               ...
                                                                               image_matches[img_idx+1], 
                                                                               camera_intrinsic,
                                                                               threshold=args.three_point_inlier_threshold)
            ## inlier visulization #####################################################
            img1 = cv2.imread(os.path.join(args.dataset_path, args.object,img_name1+".jpg"))
            img2 = cv2.imread(os.path.join(args.dataset_path, args.object,img_name2+".jpg"))
            img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
            img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
            kp1 = image_features[img_idx+1]
            kp2 = image_features[img_idx+2]
            # matches
            matching_result_img = draw_matches(img1, img2, kp1, kp2, image_matches[img_idx+1], inlier_matches_idx)
            matching_result_img_path = os.path.join(step_result_folder, 'inlier_matching_results_init_images.jpg')
            cv2.imwrite(matching_result_img_path, matching_result_img)
            # each keypoints 
            random_colors = np.random.randint(0, 256, size=(len(inlier_p1), 3)).tolist()
            vis_img1 = draw_keypoints(img1, inlier_p1, random_colors)
            vis_img2 = draw_keypoints(img2, inlier_p2, random_colors)
            vis_out1 = os.path.join(step_result_folder, 'inliers_img1.jpg')
            vis_out2 = os.path.join(step_result_folder, 'inliers_img2.jpg')
            cv2.imwrite(vis_out1, vis_img1)
            cv2.imwrite(vis_out2, vis_img2)
            #############################################################################
            points_3d, inlier_idx = triangulate_points(camera_matrix_list[img_idx+1], 
