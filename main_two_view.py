...
def args_parser():
    ...
    parser.add_argument('--matching_threshold_knn', type=float, default=0.75)
...
def main():
    ...
    if 2 in args.step and not os.path.isfile(E_result_path):
        ...
        ## inlier visulization #####################################################
        # matches
        matches_inlier = [matches[i] for i in inlier_matches_idx.astype(int)]
        matches_for_draw = [[m] for m in matches_inlier]
        matching_result_img = cv2.drawMatchesKnn(img1, kp1, img2, kp2, matches_for_draw, None, flags=2)
        matching_result_img = cv2.resize(matching_result_img, (3200, 1200), interpolation=cv2.INTER_AREA)
        matching_result_img_path = os.path.join(output_path, 'inlier_matching_results_init_images.jpg')
        cv2.imwrite(matching_result_img_path, matching_result_img)
        # each keypoints 
        def draw_keypoints(img, points, colors, radius=5):
            img_copy = img.copy()
        ...
        vis_out1 = os.path.join(output_path, 'inliers_img1.jpg')
        vis_out2 = os.path.join(output_path, 'inliers_img2.jpg')
    ...
    print("\nNumber of triangulated 3D points:", points_3d.shape[0])
